...
socket.on('seen', async (msgByUserId) => {
    try {
        ...
        if (!msgByUserId) {
            socket.emit('error', { message: 'msgByUserId is required for the seen event.' });
            return;
        }
        ...
        if (updateResult.nModified > 0) {
            io.to(senderId).emit('messages-seen', { conversationId: conversation._id, userId: userId });
        }

    } catch (error) {
        console.error('Error in seen event:', error);
        socket.emit('error', { message: 'An error occurred while updating message seen status.' });
    }
});
    ...
    try {
        ...
        socket.on('new-message', async (data) => {
            try {
                ...
                if (senderId === receiverId) {
                    socket.emit('error', { message: 'Cannot send messages to yourself.' });
                    return;
                }
